...
- 所有关联产品必须属于主档 `purchaseOrderId`，供应商必须与该订单一致，且入库数量不得超过订单行的待入库数量；产品和采购单价以订单快照为准。
- 未提交 `costUnit` 时，按 `unitPrice` 初始化成本单价。
- 正式提交成功后即按本次入库明细的增减差额回写采购订单行的 `inboundQty` 和 `inboundStatus`，并由全部订单行汇总更新采购订单的 `inboundStatus`；该回写不依赖审批是否通过。
