...
本目录是研发 Harness 的唯一入口：把需求输入、规则、实现、验证、文档和交付串成可重复执行的流程。`AGENTS.md`、`CLAUDE.md` 与各 SKILL 只负责要求 Agent 读取和执行本目录规则，不重复维护业务规范。文档分类、事实源和按需加载规则见 `后端文档规范.md`，定位入口见 `文档地图.md`。
...
- `后端文档规范.md`：后端文档分类、事实源、命名与维护责任。
- `文档地图.md`：按任务类型选择最小文档集的入口与存量映射。
- `../guide/业务模块目录规范.md`：以客户模块为基准的业务模块目录职责与新增 package 登记规则。
