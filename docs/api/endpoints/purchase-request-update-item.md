...
## 文档信息
- 领域：`purchase-request-update-item`
- 控制器：`PurchaseRequestAdminController#updateItem`
- 请求方式：`POST /erp/v1/purchase/request/updateItem`
...
| `data.data` | 是 | 编辑页回填数据 |
| `data.data.main` | 否 | 采购申请主档回填对象；未命中数据时返回默认对象 |
| `data.data.items` | 是 | 采购申请明细列表；未命中或无明细时返回空数组 |
