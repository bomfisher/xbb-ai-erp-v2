...
`purchase_order_item` 是采购订单 `purchase_order` 的子表，按 `purchase_order_id + line_no` 唯一定位行，按 `corpid + sku_id` 支持租户内 SKU 查询。`warehouse_id` 保存该采购订单行后续入库使用的仓库，由采购入库选择采购订单时回填到入库产品行。
...
| 仓库 | `warehouse_id` | 采购订单行后续入库使用的仓库 |
