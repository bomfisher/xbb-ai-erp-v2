...
- 先准备模块规格 YAML；已有规格优先复用，没有则按数据库设计资料补一份。规格字段至少包含：`moduleCode`、`moduleName`、`packageBase`、`pathStrategy`、`aggregate.aggregateName`、`aggregate.tableName`、`aggregate.fields`、`generate`。可参考 `xbb-erp-codegen/src/main/resources/examples/supplier-vendor.yaml`，以及技能目录中的 `.claude/commands/init-module/customer-module-demo.yaml`
...
- 生成结果需要符合当前项目约束：直连数据库的对象实体后缀必须是 `Entity`；直连数据库字段禁止使用布尔值，改用 `Integer`；枚举类必须以 `Enum` 结尾；接口入参统一使用 DTO，非脚本接口 DTO 继承 `BaseDTO`；`userId` 是字符串
- 创建完新module后，回写到 `docs/base/项目顶部和底部module导航.md`
...
- 第八步：回写 `docs/sql` 与 `docs/base/项目顶部和底部module导航.md`
- 第九步：执行模块级编译验证，并记录无关失败项，不顺手修 unrelated 问题

## 交付物清单
- 目标业务模块目录，例如 `xbb-erp-module-<moduleCode>`
- 模块规格 YAML，优先放在生成器示例或业务模块可追溯的位置
- 持久层代码：`Entity`、`Mapper`、`RepositoryImpl`、`Mapper XML`
- 管理端基础骨架：`admin/dto`、`admin/vo`、`controller`、`application/service`
- 建表 SQL：`docs/sql/<date>-init-<moduleCode>-module.sql`
- 导航文档回写：`docs/base/项目顶部和底部module导航.md`
