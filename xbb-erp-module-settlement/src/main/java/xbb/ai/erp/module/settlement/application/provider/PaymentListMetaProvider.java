...
@Component
public class PaymentListMetaProvider implements ListMetaProvider {

    @Override
    public String businessCode() {
        return BusinessCodeEnum.PAYMENT.getCode();
    }

    @Override
    public List<FilterField> buildFilterMeta(ListCommonQueryDTO dto) {
        return Arrays.stream(PaymentFieldEnum.values()).filter(field -> field.getFilterName() != null)
    ...
    public Map<String, ListFilterMetaPojo> buildFilterConditionMeta(ListCommonQueryDTO dto) {
        ...
        Arrays.stream(PaymentFieldEnum.values()).filter(field -> field.getFilterName() != null)
    ...
    public List<FieldEntity> buildHeaderMeta(ListCommonQueryDTO dto) {
        return Arrays.stream(PaymentFieldEnum.values()).filter(field -> field.supports(SceneTypeEnum.LIST))
    ...
    public ListMetaBundlePojo buildRowActionMeta(ListCommonQueryDTO dto) {
        ...
        ListMetaBundlePojo bundle = new ListMetaBundlePojo();
        bundle.setRowActionList(new ArrayList<>(List.of(action)));
        return bundle;
    }

    private FilterField buildFilterField(PaymentFieldEnum field) {
    ...
    private static ListFilterFieldTypeRule filterRule(PaymentFieldEnum field) {
