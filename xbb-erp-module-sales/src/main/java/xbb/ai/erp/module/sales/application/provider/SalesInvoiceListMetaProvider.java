...
@Component
public class SalesInvoiceListMetaProvider implements ListMetaProvider {

    @Override
    public String businessCode() {
        return BusinessCodeEnum.SALES_INVOICE.getCode();
    }
    ...
    public Map<String, ListFilterMetaPojo> buildFilterConditionMeta(ListCommonQueryDTO dto) {
        ...
        Arrays.stream(SalesInvoiceFieldEnum.values())
            .filter(field -> field.getFilterName() != null)
    ...
    public ListMetaBundlePojo buildTopButtonMeta(ListCommonQueryDTO dto) {
        ...
        bundle.setTopButtonList(List.of(
            ...
            new ListButtonItemPojo("AUDIT", "审核/反审核", 20, "AUDIT")
        ));
        return bundle;
    }
    ...
    public ListMetaBundlePojo buildRowActionMeta(ListCommonQueryDTO dto) {
        ...
        actions.add(action);
        actions.add(rowAction("POST", "过账", 20, "MORE"));
        actions.add(rowAction("RED_FLUSH", "红冲", 30, "MORE"));
        actions.add(rowAction("VOID", "作废", 40, "MORE"));
    ...
    private FilterField buildFilterField(SalesInvoiceFieldEnum field) {
    ...
    private static ListFilterFieldTypeRule filterRule(SalesInvoiceFieldEnum field) {
